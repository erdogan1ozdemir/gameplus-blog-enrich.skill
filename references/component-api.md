...
- **`badge_href`** rozet linkini kontrol eder. **`None` (varsayılan) = OTOMATİK:** tek/saf rozet tüm rozeti kendi kategorisine linkler; **birleşik rozet (ayraçlı: Aksiyon-Macera) HER PARÇAYI ayrı ayrı** kendi kategorisine linkler (eşleşmeyen parça düz kalır). **`False` = link YOK** (tek-tür seride stuffing önlemi, ör. hepsi FPS). **URL = tüm rozeti o URL'e** linkle. Linkli rozet `<a display:contents>` / parça-bazlı `<a color:inherit>` ile sarılır (linksizle birebir aynı yükseklik). **Dedup YOK:** eşleşen her rozet linklenir.
- Card-table satırıyla (badge=tür, meta="Stüdyo · Yıl") birebir aynı veri.

### `category_url_for(badge)` + `GFN_CATEGORY_URLS`
`badge` (ör. "Aksiyon") **tek/saf** bir GFN kategorisine fit ediyorsa kategori URL'ini döndürür, yoksa `None`. Ayraçlı birleşik rozette (`Aksiyon-Macera`) `None` döner — **birleşik rozetin parça-bazlı linklenmesini `render_game_h3_inline` (badge_href=None) kendi içinde yapar**, category_url_for'a düşmez. Çok kelimeli tekil kategoriler boşlukla yazıldığından ("Dövüş Oyunu", "Aile Dostu") eşleşir. Türkçe/case duyarsız. Linklenebilir set: strateji, aksiyon, simulasyon, dovus-oyunu, yaris, fps, mmo, macera, steam, canlandirma, moba, bagimsiz, arcade, bulmaca, basit-eglence, aile-dostu, platform, spor, ubisoft-connect, populer-oyunlar.
