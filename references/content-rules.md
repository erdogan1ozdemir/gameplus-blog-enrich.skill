...
- **Oyun GFN'de var ve birden fazla türe uyuyorsa** birleşik rozet yazabilirsin: **Aksiyon-Macera, Aksiyon-RPG, Indie-RPG** gibi.
...
## 12. Tür rozeti → GFN kategorisine iç link (dedup)
Tür rozeti markanın **linklenebilir GFN kategorilerinden** birine fit ediyorsa, rozet o kategori sayfasına iç link olur (`category_url_for(badge)` URL'i döndürür; birleşik rozette ilk parça denenir). Linklenebilir kategoriler: `strateji, aksiyon, simulasyon, dovus-oyunu, yaris, fps, mmo, macera, steam, canlandirma, moba, bagimsiz, arcade, bulmaca, basit-eglence, aile-dostu, platform, spor, ubisoft-connect, populer-oyunlar`.
- **Sayfa başına her kategoriye EN FAZLA 1 link** (link stuffing / first-link-priority israfı olmasın). Build script bir `seen` kümesi tutar; kategori zaten linklenmişse rozeti düz bırak.
- **Önce yazıdaki doğal prose linkini tercih et:** o kategori gövdede zaten bağlam içinde linkliyse rozeti linkleme (URL'i `seen`'e önceden ekle).
- Rozet linki **oyun başlığında** verilir (`render_game_h3_inline(..., badge_href=url)`). Card-table satırı zaten bölüme jump-link olduğu için orada rozet linklenmez (iç içe `<a>` geçersiz).
- GFN'de karşılığı olmayan türler (KORKU, SOULSLIKE, METROIDVANIA vb.) **düz rozet** kalır.
- İç link; `nofollow`/`target=_blank` YOK. Görünüm değişmez (rozet aynı, sadece `<a>` ile sarılı).

### Build script dedup deseni
```python
seen = set()  # gerekiyorsa yazının prose linklerini önceden ekle: seen.add(category_url_for("Platform"))
def badge_href_for(badge):
    u = category_url_for(badge)
    if u and u not in seen:
        seen.add(u); return u
    return None
# her oyun: render_game_h3_inline(anchor, name, badge, color, meta, level=lvl, badge_href=badge_href_for(badge))
```
