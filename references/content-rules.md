...
Bir oyun bir **listede** veya **kendi bölümünde** tanıtıldığında her zaman 3 veri taşır: **(1) tür etiketi, (2) yapımcı stüdyo, (3) çıkış yılı/dönemi.** Format bütün yazılarda (genel blog, listicle, etkinlik özeti, GFN) aynıdır.
- **Oyunun kendi başlığı/bölümü varsa** (listicle oyunları, State of Play gibi etkinlik özetlerindeki her oyun, haftanın öne çıkan oyunu): başlığı `render_game_h3_inline(anchor, "Oyun Adı", "TÜR", renk, "Stüdyo · Yıl")` ile ver. **Düz `<h3 id="x">Oyun Adı</h3>` BIRAKMA.** Yazarın başlık metnini koru, sadece tür rozeti + stüdyo·yıl ekle.
- **Card-table satırı:** `badge` = tür, `meta` = "Stüdyo · Yıl". Başlık (inline H3) ile card-table verisi **birebir aynı** olmalı.
...
- **Tür taksonomisi kural 5 ile tutarlı:** aynı tür → aynı etiket → aynı renk; tüm yazılarda aynı. (Tematik istisna: bir tekno-tanıtım yazısında rozet türün yerine temayı gösterebilir, ör. DLSS açıklayıcısında "DLSS"/"RAY TRACING"; stüdyo · yıl yine bulunur.)
